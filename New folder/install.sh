#! /bin/bash
clear
printf '\033]2;Hak9 Installer\a'

echo -e ""
echo -e "░██     ░██            ░██        ░██████ \e[1;33mv4.0.0\e[0m"  
echo -e "░██     ░██            ░██       ░██   ░██ "
echo -e "░██     ░██  ░██████   ░██    ░██░██   ░██ " 
echo -e "░██████████       ░██  ░██   ░██  ░███████ " 
echo -e "░██     ░██  ░███████  ░███████        ░██ "
echo -e "░██     ░██ ░██   ░██  ░██   ░██ ░██   ░██ "
echo -e "░██     ░██  ░█████░██ ░██    ░██ ░██████ " 
echo -e ""
echo -e "\e[1;33mBy Ships2024\e[0m"
echo -e "Visit us on github:"
echo -e "==> \e[1;33mhttps://github.com/Ships2024\e[0m"                                                           
echo -e "==> \e[1;33mHak9 Framework\e[0m"
echo -e "Press \e[1;33many key\e[0m to install Hak9"
read -n 1
clear
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
if [[ "$DIR" != "/root/hak9" ]]
then
	echo -e "You didn't follow the github's install instructions.Hak9 will try to do it for you..."
	sleep 4
	if [[ -d /root/hak9 ]]
	then
		rm -r /root/hak9
	fi
	mkdir /root/hak9
	cp -r "$DIR"/* /root/hak9
	chmod +x /root/hak9/install.sh
	gnome-terminal -- "bash /root/hak9/install.sh"
fi
echo -e "Setting up folders..."
sleep 1
echo -e "Installing Framework"
sleep 2
chmod +x /root/hak9/hak1
chmod +x /root/hak9/hak2
chmod +x /root/hak9/hak3
chmod +x /root/hak9/hak31
chmod +x /root/hak9/hak9
chmod +x /root/hak9/hak4
chmod +x /root/hak9/hak41
chmod +x /root/hak9/hak42
chmod +x /root/hak9/hak43
chmod +x /root/hak9/hs/h9131.sh
chmod +x /root/hak9/hs/h9132.sh
chmod +x /root/hak9/hs/h9133.sh
chmod +x /root/hak9/uninstall.sh
clear
echo -e "Copying script to /bin/hak9"
sleep 2
mkdir /bin/hak9
cd /root/hak9
cp /root/hak9/hak9 /bin/hak9
cp /root/hak9/hak1 /bin/hak9
cp /root/hak9/hak2 /bin/hak9
cp /root/hak9/hak3 /bin/hak9
cp /root/hak9/hak31 /bin/hak9
cp /root/hak9/hak4 /bin/hak9
cp /root/hak9/hak41 /bin/hak9
cp /root/hak9/hak42 /bin/hak9
cp /root/hak9/hak43 /bin/hak9
clear
#required for gui
apt-get -y install ncurses-dev
clear
if [[ ! -d /root/handshakes ]]
then
	mkdir /root/handshakes
	echo -e "Made /root/handshake directory"
else
	echo -e "/root/handshakes directory detected- Good."
fi
if [[ ! -d /root/wordlists ]]
then
	mkdir /root/wordlists
	echo -e "Made /root/wordlists directory"
else
	echo -e "/root/wordlists directory detected- Good."
fi
while true
do
clear
echo -e "Are you \e[1;33mu\e[0mpdating or \e[1;33mi\e[0mnstalling the Framwork?(\e[1;33mu\e[0m/\e[1;33mi\e[0m): "
echo -e "Lets Install the Framework."
read UORI
if [[ "$UORI" = "u" ]]
then 
	clear
	echo -e "? Lets update the Framework!"
	sleep 3
	break
elif [[ "$UORI" = "i" ]]
then
	clear
	BASHCHECK=$(cat ~/.bashrc | grep "bin/hak9")
	if [[ "$BASHCHECK" != "i" ]]
	then
		echo -e "Hak9 Framework has been successfully installed"
		sleep 3
	fi
	echo -e "Adding Hack9 to PATH so you can access it from anywhere"
	sleep 1



	export PATH=/bin/hak9:$PATH
	sleep 2
	echo "export PATH=/bin/hak9:$PATH" >> ~/.bashhrc
	sleep 2
	echo "export PATH=/bin/hak9:$PATH" >> ~/.zshrc
	clear
	break
fi
done
clear
echo -e "Added Hack9 to Path"
sleep 1
clear
echo -e "Open a NEW terminal and type 'hak9' to launch framework"
sleep  4
gnome-terminal -- hak9
exit
